...
def add_game_to_cart(username, game):
    """Add a game to the user's cart."""
    akuns[username]['cart'].append(game)
    print(f"Your game '{game}' has been added to the cart\n")
...
def delete_from_cart(username):
    """Delete a game from the user's cart."""
    cart = akuns[username]['cart']
    if not cart:
            ...
            while True:
                ...
                if status == "1":
                    ...
                    if 0 <= nomor_game < len(game_yang_ada):
                        ...
                        add_game_to_cart(Username, selected_game)
                    else:
                        print("Invalid game number.\n")
